...
<body style="margin:0;padding:0;background-color:#f6f6f6;font-family:Arial,Helvetica,sans-serif;color:#333333;">
  <span class="preheader">Set up your SupplySystem account for {{ $user->name }}.</span>
        ...
        <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="680" style="max-width:680px;width:100%;background:#ffffff;border:1px solid #e5e5e5;">
              ...
              <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%">
                <tr>
                  ...
                  <td style="vertical-align:middle;padding-left:12px;">
                    <div style="font-size:18px;font-weight:600;line-height:1;color:{{ $brandText ?? '#ffffff' }};">Web-Based Supply and Property Management System</div>
                    <div style="font-size:12px;opacity:0.95;color:{{ $brandText ?? '#ffffff' }};">Account Setup</div>
                  </td>
          ...
          <tr>
            <td style="padding:22px 24px;">
              ...
              <p style="margin:0 0 14px 0;color:#666;font-size:14px;">An account has been created for you. To get started, set your password using the button below. The link will expire in <strong>{{ $expires ?? '24 hours' }}</strong>.</p>

              <p style="margin:14px 0;">
                <a href="{{ $url }}" class="button" style="display:inline-block;padding:10px 18px;background:{{ $brandPrimary ?? '#3b82f6' }};color:{{ $brandText ?? '#ffffff' }};text-decoration:none;border-radius:4px;font-weight:600;">Set Up Your Account</a>
              </p>

              @if(!empty($additionalInstructions))
                <div style="margin-top:8px;color:#666;font-size:13px;">{{ $additionalInstructions }}</div>
              @endif
